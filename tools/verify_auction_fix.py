#!/usr/bin/env python3
"""交易日09:26自动验收竞价封单修复：915/920应有真实竞价封单且队列随时间增长。"""
import glob
import sqlite3
...
import csv as _csv
for f in glob.glob(f"/opt/jihejingjia/snapshots/{day_compact}/{day_compact}_0915*涨停*.csv"):
...
files = sorted(glob.glob(f"/opt/jihejingjia/snapshots/{day_compact}/*.csv"))
